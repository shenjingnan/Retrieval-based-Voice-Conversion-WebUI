...
import { formatGib, pickProductName, randomHex } from '@/lib/domain'
...
/** 手写进度条（与任务监视区的进度条同款视觉）；percent 为 null 表示无数据：
 *  不渲染填充、仅脉冲占位（与进度未知态同款表达） */
...
/** 系统资源卡（常驻，与任务无关）：GPU/内存/CPU/硬盘一行一条。stats 为 null（从未
 *  成功拉到过）整卡降级为提示文案；各采集项的 null 在行内降级为「不可用」，互不影响 */
function SystemResourcePanel({ stats, stale }: { stats: SystemStats | null; stale: boolean }) {
  return (
  ...
  return (
    <Card>
      ...
      <CardContent className="flex flex-col gap-6">
        ...
        {/* 系统资源（常驻：不放进 watched 门控，无任务也要显示；GPU 不可用不报错） */}
